...
export TENSORFLOW_DIR="${SCRIPT_DIR}/../../../third_party/tensorflow"
TENSORFLOW_LITE_DIR="${SCRIPT_DIR}/../.."

export PACKAGE_VERSION="${RELEASE_VERSION:-1.2.0}"
export PROJECT_NAME=${WHEEL_PROJECT_NAME:-ai_edge_litert}

BUILD_DIR=`pwd`
TENSORFLOW_TARGET=$1

mkdir -p "${BUILD_DIR}/litert_pip"
mkdir -p "${BUILD_DIR}/litert_pip/ai_edge_litert"
cp -r "${TENSORFLOW_LITE_DIR}/tools/pip_package/debian" \
      "${TENSORFLOW_LITE_DIR}/tools/pip_package/MANIFEST.in" \
      "${TENSORFLOW_LITE_DIR}/python/interpreter_wrapper" \
      "${BUILD_DIR}/litert_pip"
cp  "${TENSORFLOW_LITE_DIR}/tools/pip_package/setup_with_binary.py" "${BUILD_DIR}/litert_pip/setup.py"
cp "${TENSORFLOW_LITE_DIR}/python/interpreter.py" \
   "${TENSORFLOW_LITE_DIR}/python/metrics/metrics_interface.py" \
   "${TENSORFLOW_LITE_DIR}/python/metrics/metrics_portable.py" \
   "${BUILD_DIR}/litert_pip/ai_edge_litert"
echo "__version__ = '${PACKAGE_VERSION}'" >> "${BUILD_DIR}/litert_pip/ai_edge_litert/__init__.py"
echo "__git_version__ = '$(git -C "${TENSORFLOW_DIR}" describe)'" >> "${BUILD_DIR}/litert_pip/ai_edge_litert/__init__.py"
# Build python interpreter_wrapper.

cmake --build . --verbose -j ${BUILD_NUM_JOBS} -t _pywrap_tensorflow_interpreter_wrapper
cmake --build . --verbose -j ${BUILD_NUM_JOBS} -t pywrap_genai_ops
cd "${BUILD_DIR}"
...
cp "${BUILD_DIR}/_pywrap_tensorflow_interpreter_wrapper${LIBRARY_EXTENSION}" \
   "${BUILD_DIR}/litert_pip/ai_edge_litert"
cp "${BUILD_DIR}/experimental/genai/pywrap_genai_ops${LIBRARY_EXTENSION}" \
   "${BUILD_DIR}/litert_pip/ai_edge_litert"
...
chmod u+w "${BUILD_DIR}/litert_pip/ai_edge_litert/_pywrap_tensorflow_interpreter_wrapper${LIBRARY_EXTENSION}"
chmod u+w "${BUILD_DIR}/litert_pip/ai_edge_litert/pywrap_genai_ops${LIBRARY_EXTENSION}"

# Build python wheel.
cd "${BUILD_DIR}/litert_pip"
case "${TENSORFLOW_TARGET}" in
  armhf)
...
find "${BUILD_DIR}/litert_pip/dist"
...
if [[ "${DEB_VERSION}" != "${PACKAGE_VERSION}" ]]; then
  cat << EOF > "${BUILD_DIR}/litert_pip/debian/changelog"
...
$(<"${BUILD_DIR}/litert_pip/debian/changelog")
EOF
fi
...
cat "${BUILD_DIR}//litert_pip/debian/changelog"
